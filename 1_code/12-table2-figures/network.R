...
data <-
  readxl::read_xlsx("3_data_analysis/some_figures/table2/data2.xlsx") 

data2 <-
  readxl::read_xlsx("3_data_analysis/some_figures/table2/Table 2 (4).xlsx")
...
data <-
data %>% 
  dplyr::left_join(data2[,c("Metabolite", "qvalue")],
                   by = "Metabolite") %>% 
  dplyr::select(-c(qvalue.x)) %>% 
  dplyr::rename(qvalue = qvalue.y) %>% 
  dplyr::mutate(qvalue = as.numeric(qvalue))

dir.create("3_data_analysis/some_figures/table2/network")
setwd("3_data_analysis/some_figures/table2/network")
