...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/multi_omics/k_means_depression_index/")
...
temp_data %>% 
  ...
  ggplot(aes(Time, bdi_total)) +
  geom_path(aes(color = subject_id, group = subject_id),
            show.legend = FALSE) +
  geom_boxplot(aes(Time, bdi_total), color = "black",
               fill = "transparent") +
...
# ggsave(plot1, filename = "depression_distributation1.pdf", width = 10, height = 7)
...
plot2 = 
  ...
  geom_point(aes(fill = subject_id),
             ...
             shape = 21,
             show.legend = FALSE) +
  geom_smooth(aes(fill = subject_id), 
              se = TRUE,
              linetype = -1,
              show.legend = FALSE) +
...
# ggsave(plot2, filename = "depression_distributation2.pdf", width = 10, height = 7)
...
temp_data <-
  sample_info %>% 
  dplyr::filter(subject_id %in% temp_subject_id) %>% 
  dplyr::select(subject_id, Time, bdi_total) %>% 
  tidyr::pivot_wider(names_from = "Time", values_from = "bdi_total") %>% 
...
temp_data = temp_data[remain_idx,]

tp <- sort(unique(colnames(temp_data)))
...
###
cluster_color <-
  ggsci::pal_jama()(n = 7)[1:2]

names(cluster_color) <- as.character(1:2)
